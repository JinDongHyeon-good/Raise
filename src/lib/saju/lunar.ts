/**
 * 음력 ↔ 양력 변환 (1900-2100).
 * 표준 lunarInfo 비트 테이블 사용. 기준일: 1900-01-31(양력) = 1900-01-01(음력).
 * 사주 계산은 최종적으로 양력(그레고리력) 일자를 사용하므로, 음력 입력을 양력으로 변환하는 데 쓴다.
 */

// prettier-ignore
const LUNAR_INFO: number[] = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
  0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
  0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
  0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
  0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
  0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
  0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
  0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6,
  0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
  0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0,
  0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
  0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
  0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
  0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
  0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
  0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
  0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
  0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
  0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0, 0x0d150, 0x0f252,
  0x0d520,
];

const BASE_YEAR = 1900;
/** 1900-01-31 (양력) = 1900-01-01 (음력) 의 정수 일 수 기준(임의 epoch) */
const BASE_DATE = Date.UTC(1900, 0, 31);
const DAY_MS = 86_400_000;

function leapMonth(year: number): number {
  return LUNAR_INFO[year - BASE_YEAR] & 0xf;
}

function leapDays(year: number): number {
  if (leapMonth(year) === 0) return 0;
  return LUNAR_INFO[year - BASE_YEAR] & 0x10000 ? 30 : 29;
}

/** 평달(1~12)의 일수 */
function monthDays(year: number, month: number): number {
  return LUNAR_INFO[year - BASE_YEAR] & (0x10000 >> month) ? 30 : 29;
}

function lunarYearDays(year: number): number {
  let sum = 0;
  for (let m = 1; m <= 12; m += 1) sum += monthDays(year, m);
  return sum + leapDays(year);
}

export type SolarDate = { year: number; month: number; day: number };

/**
 * 음력 → 양력 변환.
 * @param isLeap 윤달 여부
 */
export function lunarToSolar(
  lYear: number,
  lMonth: number,
  lDay: number,
  isLeap = false,
): SolarDate {
  if (lYear < BASE_YEAR || lYear > BASE_YEAR + LUNAR_INFO.length - 1) {
    throw new Error("지원 범위를 벗어난 음력 연도입니다.");
  }

  let offset = 0;
  for (let y = BASE_YEAR; y < lYear; y += 1) offset += lunarYearDays(y);

  const leap = leapMonth(lYear);
  for (let m = 1; m < lMonth; m += 1) {
    offset += monthDays(lYear, m);
    if (m === leap) offset += leapDays(lYear);
  }
  // 윤달 자체를 지정한 경우, 해당 평달을 지난 뒤 윤달 일수를 더한다
  if (isLeap && leap === lMonth) {
    offset += monthDays(lYear, lMonth);
  }
  offset += lDay - 1;

  const solar = new Date(BASE_DATE + offset * DAY_MS);
  return {
    year: solar.getUTCFullYear(),
    month: solar.getUTCMonth() + 1,
    day: solar.getUTCDate(),
  };
}

export function isLunarYearSupported(year: number): boolean {
  return year >= BASE_YEAR && year <= BASE_YEAR + LUNAR_INFO.length - 1;
}

export function lunarLeapMonth(year: number): number {
  if (!isLunarYearSupported(year)) return 0;
  return leapMonth(year);
}
