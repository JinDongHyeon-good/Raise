...
import { Sun, Sparkles, HeartHandshake, MessagesSquare, User, ArrowRight } from "lucide-react";

const FEATURES = [
  { id: "today", href: "/dashboard/today" as const, icon: Sun, accent: true },
  { id: "saju", href: "/dashboard/saju" as const, icon: Sparkles, accent: true },
  { id: "gunghap", href: "/dashboard/gunghap" as const, icon: HeartHandshake, accent: true },
  { id: "community", href: "/dashboard/board" as const, icon: MessagesSquare, accent: false },
  { id: "mypage", href: "/mypage" as const, icon: User, accent: false },
] as const;
          ...
          {FEATURES.map((feature) => {
            ...
            return (
              <LocaleLink
                key={feature.id}
                href={feature.href}
                className={`group rounded-2xl border p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_36px_-18px_rgb(42_33_80_/0.5)] sm:p-6 ${
                  feature.accent
                    ? "border-[var(--piclick-line)] bg-white hover:border-[var(--piclick-green)]"
                    : "border-[var(--piclick-line)] bg-[var(--piclick-beige)]/40 hover:border-[var(--piclick-green)]/50"
                }`}
              >
